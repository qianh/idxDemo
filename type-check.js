...
const filesToCheck = process.argv.join(' '); // <--- YOU PROBABLY WANT TO CHANGE
...
exec(`tsc ${filesToCheck} ${optionsString} --noEmit`, (err, stdout) => {
  const results = stdout
    .split('\n')

    // Filter out excludes
    .filter(line => (tsConfig.exclude ? !tsConfig.exclude.some(exclude => line.startsWith(exclude)) : true))
